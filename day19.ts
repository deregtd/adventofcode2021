...
function generateScannerRotations(scanner: number[][]): number[][][] {
    ...
    // for (let yRot=0; yRot<4; yRot++) {
    //     const yRotSet = scanner.map(pt => rotatePoint(pt, 0, yRot, 0));
    //     for (let xRot=0; xRot<4; xRot++) {
    //         const set = yRotSet.map(pt => rotatePoint(pt, xRot, 0, 0));
    //         ret.push(set);
    //     }
    // }

    // for (let zRot=1; zRot<3; zRot+=2) {
    //     const zRotSet = scanner.map(pt => rotatePoint(pt, 0, 0, zRot));
    //     for (let xRot=0; xRot<4; xRot++) {
    //         const set = zRotSet.map(pt => rotatePoint(pt, xRot, 0, 0));
    //         ret.push(set);
    //     }
    // }

    // const setNegX = scanner.map(pt => rotatePoint(pt, , 0, 0));

    // // negative x
    //     for (let zRot=1; zRot<3; zRot+=2) {
    //         const set = scanner.map(pt => rotatePoint(pt, xRot, 0, zRot));
    //         ret.push(set);
    //     }
    // }
    // for (let xRot=0; xRot<4; xRot++) {
    //     for (let yRot=0; yRot<4; yRot++) {
    //         let set = scanner.map(pt => rotatePoint(pt, xRot, 0, 0));
    //         set = set.map(pt => rotatePoint(pt, 0, yRot, 0));
    //         ret.push(set);
    //     }
    //     for (let zRot=1; zRot<3; zRot+=2) {
    //         let set = scanner.map(pt => rotatePoint(pt, xRot, 0, 0));
    //         set = scanner.map(pt => rotatePoint(pt, 0, 0, zRot));
    //         ret.push(set);
    //     }
    // }
    return ret;
...
function diffEq(diff1: number[], diff2: number[]): boolean {
...
let ptLookup: {[key: number]: {[key: number]: {[key: number]: number}}} = {};
...
const linkOffset: {[key: number]: {[key: number]: number[]}} = {};

// for (let i=0; i<1; i++) {
//     for (let h=i + 1; h<2; h++) {
for (let i=0; i<scanners.length; i++) {
    for (let h=i + 1; h<scanners.length; h++) {
        const scanRotsI = scannersRot[i];
        const scanRotsH = scannersRot[h];

        if (!(i in lockedRotation)) {
            throw 'noILock';
        }
        const scanRotI = scanRotsI[lockedRotation[i]];
        ...
        for (let hRot=0; hRot<scanRotsH.length; hRot++) {
            ...
            if (count >= 12) {
                ...
                console.log('bestMatch of ' + i + '/' + h);
                for (const pair of Object.entries(subLookup)) {
                    const iPt = Number(pair[0]), hPt = pair[1];

                    addLookup(i, h, iPt, hPt);
                }
            }
        }
    }
...
function addLookup(i: number, h: number, iPt: number, hPt: number): void {
    if (ptLookup[i][iPt] === undefined) {
        ptLookup[i][iPt] = {[h]: hPt};
    } else if (!(h in ptLookup[i][iPt])) {
        for (const h2raw in ptLookup[i][iPt]) {
            const h2 = Number(h2raw);
            const h2Pt = ptLookup[i][iPt][h2];
            addLookup(h2, i, h2Pt, iPt);
        }
        ptLookup[i][iPt][h] = hPt;
    } else {
        return;
    ...
    addLookup(h, i, hPt, iPt);
}

// form the whole lookup

let count = 0;
for (let i=0; i<scanners.length; i++) {
    for (let pt=0; pt<scanners[i].length; pt++) {
        let found = false;
        if (ptLookup[i][pt] !== undefined) {
            for (const h in ptLookup[i][pt]) {
                if (Number(h) < i) {
                    found = true;
                    break;
                }
