...
class SFPair {
    ...
    clone(): SFPair {
        return new SFPair(
            typeof this.left === 'number' ? this.left : this.left.clone(),
            typeof this.right === 'number' ? this.right : this.right.clone(),
            undefined);
    }
...
for (let a=0; a<rows.length; a++) {
    for (let b=0; b<rows.length; b++) {
        ...
        const added = rows[a].clone().add(rows[b].clone());
        const mag = added.magnitude();
        biggestMag = Math.max(mag, biggestMag);
    }
}

console.log(biggestMag);