...
function totalVersions(packet: Packet): number {
    let versionTotal = packet.version;
    if (packet.typeId !== TypeIds.Literal) {
        (packet as PacketOperator).packets.forEach(subPacket => {
            versionTotal += totalVersions(subPacket);
        })
    }
    return versionTotal;
}
...
while (!reader.eof()) {
    ...
    packets.push(packet);
    versionTotal += totalVersions(packet);
