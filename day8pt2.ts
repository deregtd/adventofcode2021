const input = `dfgabce cadfgb cefa ca aecbg dfcegb geabd ecbfg cab agcfbe | egbfadc dbgae gcfeb abgdfc
...
const rows = input.split('\n').map(r => r.split(' | ').map(ri => ri.split(' ').map(rii => rii.split('').sort().join(''))));
...
function removeFrom(base: string, toRemove: string): string {
    for (let i = 0; i < base.length;) {
        if (toRemove.indexOf(base.substr(i, 1)) !== -1) {
            base = base.substr(0, i) + base.substr(i+1);
        } else {
            i++;
        }
    }
...
const output = rows.map(row => {
    const cf = row[0].filter(a => a.length === 2)[0];   // 1
    const acf = row[0].filter(a => a.length === 3)[0];  // 7

    console.log('cf: ' + cf);
    console.log('acf: ' + acf);

    const a = removeFrom(acf, cf);
    if (a.length !== 1) {
        throw 'a';
    }
    ...
    const bcdf = row[0].filter(a => a.length === 4)[0]; // 4
    const bd = removeFrom(bcdf, cf);
    if (bd.length !== 2) {
        throw 'bd';
    }

    const abcdefg = row[0].filter(a => a.length === 7)[0]; // 8
    const aeg = removeFrom(abcdefg, bcdf);
    const eg = removeFrom(aeg, a);
    if (eg.length !== 2) {
        throw 'eg';
    }

    // 0/6/9 are 6 long
    const sixLongs = row[0].filter(a => a.length === 6);
    const acefg = a + cf + eg;
    const abcefg = sixLongs.find(sixOne => {
        // 0: 'abcefg', -> b
        // 6: 'abdefg', -> bd
        // 9: 'abcdfg', -> bd
        const noACEFG = removeFrom(sixOne, acefg);
        return (noACEFG.length === 1);
    })!;

    const b = removeFrom(abcefg, acefg);
    if (b.length !== 1) {
        throw 'b';
    }

    const d = removeFrom(bd, b);
    if (d.length !== 1) {
        throw 'd';
    }

    const abdcf = a + bd + cf;
    const abcdfg = sixLongs.find(sixOne => {
        if (sixOne === abcefg) {
            return false;
        }
        // 6: 'abdefg', -> eg
        // 9: 'abcdfg', -> g
        const noABDCF = removeFrom(sixOne, abdcf);
        return (noABDCF.length === 1);
    })!;

    const g = removeFrom(abcdfg, abdcf);
    if (g.length !== 1) {
        throw 'g';
    }

    const e = removeFrom(eg, g);

    // 2/3/5 are 5 long
    const fiveLongs = row[0].filter(a => a.length === 5);
    const adeg = a + d + e + g;
    const acdeg = fiveLongs.find(fiveOne => {
        const noADEG = removeFrom(fiveOne, adeg);
        // 2: 'acdeg',  // c
        // 3: 'acdfg',  // cf
        // 5: 'abdfg',  // bf
            return (noADEG.length === 1);
    })!;
    const c = removeFrom(acdeg, adeg);
    if (c.length !== 1) {
        throw 'c';
    }

    const f = removeFrom(cf, c);
    if (f.length !== 1) {
        throw 'f';
    }
    ...
    const outputFixed = row[1].map(el => el.split('').map(c => ((mapChars as any)[c] as string)).sort().join(''));
    console.log(outputFixed);
    const chars = outputFixed.map(o => {
        for (let i=0; i<=9; i++) {
            if ((digits as any)[i] === o) {
                return i;
            }
        }
    }).join('');
...
console.log(output.reduce((cur, prev) => cur + prev));
